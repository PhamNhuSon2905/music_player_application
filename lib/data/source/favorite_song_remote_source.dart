...
class FavoriteSongRemoteDataSource {
  ...
  Future<List<FavoriteSong>> fetchFavoritesByUser(int userId) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'Failed to load favorite songs: ${response.statusCode} - ${response.body}',
      );
    }
  }

  // thêm bài hát yêu thích theo userId
  Future<void> addFavorite(int userId, String songId) async {
    final response = await _apiClient.post(
      '/api/favorites/add',
      body: {'userId': userId, 'songId': songId},
    );
    if (response.statusCode != 200) {
  ...
  // xóa bài hát yêu thích theo userId
